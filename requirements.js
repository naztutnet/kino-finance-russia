(() => {
  ...
  const official = {
    ...
    kinoprime: 'https://www.kinoprimefoundation.com/#rec98743691',
    forpost: 'https://forpost.center/',
    krasnoyarskDoc: 'https://krskstate.ru/kraygrants/dok_kino/uslovia/0/id/77980',
    spbNko: 'https://www.gov.spb.ru/static/writable/ckeditor/uploads/2026/04/21/20/%D0%9E%D0%B1%D1%8A%D1%8F%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5_%D0%BE%D0%B1_%D0%BE%D1%82%D0%B1%D0%BE%D1%80%D0%B5.pdf'
  };
  ...
  const rules = [
    ...
    {
      test: text => /форпост/i.test(text),
      status: 'Требует отдельной проверки официального регламента',
      documents: [
        'Нет данных по точному перечню документов и форматам.',
        'Искать в разделе поддержки и в положении о текущем отборе на сайте центра «Форпост».'
      ],
      submit: 'Нет данных по точной форме подачи и сроку нового отбора. Проверять официальный сайт и контакты центра.',
      restrictions: 'Авторы и производители документальных фильмов; точные организационно-правовые требования необходимо сверить в положении.',
      links: [['Официальный сайт центра «Форпост»', official.forpost]]
    },
    {
      test: text => /красноярск/i.test(text) && /докум/i.test(text),
      status: 'Требует отдельной проверки официального регламента',
      documents: [
        'Нет данных по полному комплекту и форматам в базе.',
        'Перечень приложений и требования к смете искать в официальных условиях грантовой программы.'
      ],
      submit: 'Подача через государственную систему предоставления мер финансовой поддержки. Приём 2026 года завершён.',
      restrictions: 'Тип заявителя и требования к регистрации необходимо сверить в официальном извещении программы.',
      links: [['Условия грантовой программы', official.krasnoyarskDoc]]
    },
    {
      test: text => /санкт-петербург/i.test(text) && /нко/i.test(text),
      status: 'Требует отдельной проверки официального регламента',
      documents: [
        'Заявка и комплект документов по объявлению об отборе.',
        'Точный перечень, формы, электронные форматы и требования к смете приведены в официальном PDF-объявлении.'
      ],
      submit: 'Электронная подача через государственную систему предоставления мер финансовой поддержки. Приём 2026 года завершён.',
      restrictions: 'Социально ориентированные некоммерческие организации, реализующие проекты в области культуры и искусства; точные критерии — в объявлении.',
      links: [['Объявление об отборе (PDF)', official.spbNko]]
    },
    {
